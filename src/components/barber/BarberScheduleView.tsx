...
export const BarberScheduleView: React.FC<BarberScheduleViewProps> = ({ barbershop }) => {
  ...
  const [viewingProofApt, setViewingProofApt] = useState<Appointment | null>(null);
  ...
  return (
    <div className="space-y-6" id="barber-schedule-view">
                ...
                <div className="space-y-1">
                  ...
                  {apt.pixProofUrl && (
                    <div className="pt-1">
                      <button
                        type="button"
                        onClick={() => setViewingProofApt(apt)}
                        className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/30 text-[11px] font-bold transition cursor-pointer"
                      >
      ...
      {/* PIX Proof Viewer Modal */}
      {viewingProofApt && viewingProofApt.pixProofUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {viewingProofApt.clientName} • {viewingProofApt.serviceName} ({formatCurrency(viewingProofApt.servicePrice)})
                </p>
              </div>
              <button
                type="button"
                onClick={() => setViewingProofApt(null)}
            ...
            <div className="max-h-[60vh] overflow-auto rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-950 flex items-center justify-center p-2">
              <img
                src={viewingProofApt.pixProofUrl}
                alt="Comprovante de PIX"
                className="max-w-full h-auto rounded-xl object-contain"
              />
            </div>

            {viewingProofApt.pixTransactionCode && (
              <div className="text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/60 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700">
                <strong>Código/Autenticação:</strong> {viewingProofApt.pixTransactionCode}
              </div>
            )}

            <div className="flex items-center justify-between gap-3 pt-2">
              {viewingProofApt.status === 'pending_pix' ? (
                <button
                  type="button"
                  onClick={() => {
                    confirmAppointmentPix(viewingProofApt.id);
                    setViewingProofApt(null);
                  }}
                  className="flex-1 py-2.5 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-md transition flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  <CheckCircle2 className="w-4 h-4" />
                  Aprovar e Confirmar Horário
                </button>
              ) : (
              ...
              <button
                type="button"
                onClick={() => setViewingProofApt(null)}
                className="py-2.5 px-4 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-xl transition cursor-pointer"
              >
                Fechar
              </button>
            </div>
